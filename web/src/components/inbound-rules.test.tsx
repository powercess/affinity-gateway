...
function setup(fail = false) {
 ...
 const fetch = vi.fn(async (url: string, init?: RequestInit) => {
  ...
  if (init?.method === "PUT") return fail ? new Response("rules changed; reload before saving", {status: 409}) : Response.json({items: [{...JSON.parse(init.body as string), revision: 1}], sources: {main: "saved"}});
  return Response.json({items: [initial], sources: {main: "default"}});
 });
...
it("saves independent source/strip flags and headers-only mode with revision", async () => {
 ...
 await user.selectOptions(screen.getByLabelText("会话识别策略"), "headers_only");
...
it("keeps draft on conflict and can discard changes", async () => {
 ...
 expect(screen.getByLabelText("会话识别策略")).toHaveValue("headers_only");
 await user.click(screen.getByRole("button", {name: "放弃修改"}));
 expect(screen.getByLabelText("会话识别策略")).toHaveValue("combined");
});
...
it("shows summaries first and retains the draft when the drawer closes", async () => {
 ...
 expect(screen.queryByLabelText("会话识别策略")).not.toBeInTheDocument();
 ...
 expect(screen.getByText("请求头 + metadata")).toBeVisible();
 await user.click(trigger);
 expect(screen.getByLabelText("会话识别策略")).toHaveValue("headers_only");
});
it("metadata preset keeps strict parsing and strips ignored identity headers", async () => {
 const {user, fetch} = setup();
 await user.click(await screen.findByRole("button", {name: "配置入站 main"}));
 await user.selectOptions(screen.getByLabelText("会话识别策略"), "metadata");
 await user.click(screen.getByRole("button", {name: "保存入站规则"}));
 const call = fetch.mock.calls.find(([, init]) => init?.method === "PUT")!;
 expect(JSON.parse(call[1]!.body as string)).toMatchObject({revision: 0, mode: "strict", metadata: true, conversation: false, cache_key: false, headers: [{name: "X-Session-Id", enabled: false, strip: true}]});
});

it("switches metadata back to the previous combined rules without saving implicitly", async () => {
 const {user, fetch} = setup();
 await user.click(await screen.findByRole("button", {name: "配置入站 main"}));
 await user.selectOptions(screen.getByLabelText("会话识别策略"), "metadata");
 expect(screen.getByRole("region", {name: "当前生效规则"})).toHaveTextContent("当前生效请求头 + metadata");
 expect(screen.getByRole("region", {name: "当前生效规则"})).toHaveTextContent("入口默认规则");
 expect(screen.getByLabelText("X-Session-Id 参与识别")).not.toBeChecked();
 await user.selectOptions(screen.getByLabelText("会话识别策略"), "combined");
 expect(screen.getByLabelText("X-Session-Id 参与识别")).toBeChecked();
 expect(screen.getByRole("button", {name: "保存入站规则"})).toBeDisabled();
 expect(fetch.mock.calls.some(([, init]) => init?.method === "PUT")).toBe(false);
});
it("shows the saved effective strategy and provenance after saving", async () => {
 const {user} = setup();
 await user.click(await screen.findByRole("button", {name: "配置入站 main"}));
 await user.selectOptions(screen.getByLabelText("会话识别策略"), "metadata");
 await user.click(screen.getByRole("button", {name: "保存入站规则"}));
 expect(screen.getByText("仅使用 metadata 会话")).toBeVisible();
 await user.click(screen.getByRole("button", {name: "配置入站 main"}));
 expect(screen.getByRole("region", {name: "当前生效规则"})).toHaveTextContent("控制台保存规则");
 expect(screen.getByRole("region", {name: "当前生效规则"})).toHaveTextContent("当前没有未保存修改");
});
